# Helana_-_Margrethe
  DAM Exam 2026 by Helena and Cecilie Margrethe

*About the repository*
  This repository contains our exam portfolio for the Digital Archives and Methods F26 course. The portfolio consists of selected assignments completed throughout the semester, as well as our final project.


*Repository Stucture*
  The portfolio includes the following assignments: 

  *Week 8* Regex / OpenRefine Assignment 

  *Week 11* Danish Kings Assignment 

  *Week 13* Visualisation Assignment 

  *Week 17* Digital Archives Reflection Assignment 

  *Final Project* Dystopian Literature and Societal Anxiety during the Second World War and the Cold War


*Final Project*
  In our final project, we investigate how global conflicts influenced dystopian literature during the Second World War and the emergence of the Cold War. Using R, we compare major historical events with the publication and themes of central dystopian novels, focusing specifically on *Animal Farm* and *Nineteen Eighthy-Four* by George Orwell. 
  We conduct a sentiment analysis to examine how themes such as surveillance, fear, propaganda, oppression, and political control reflect societal anxieties during periods of conflicts.
  Through this project, we aim to explore how literature can function as a reflection of society and historical tensions. 

*Hypothis*
  We expect the sentiment analysis to show a predominance of negative emotional language in both novels, especially passages connected to oppression, propaganda, violence, and control.
  These patterns may support the interpretation that Orwell's fiction reflects anxieties about totalitarianism and the ideological conflicts that shaped the Cold War.

*Methods and Tools*
  The project primarily uses: R, MANGLER 


*Authors*
  Helena Hymøller Jensen
  Cecilie Margrethe Nielsen 

*Course Information*
  Digital Archives and Methods F26
  Aarhus University 
  BA History – Second semester   
